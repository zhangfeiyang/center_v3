// assume energy resolution for e+, sigma^2 = a^2*Erec+b^2*Erec^2
// 1255.02	0.142111	0.142111	36.98	0.100211 for 0 MeV e+
// Erec(0) = 1255.02/1.3056438e3
// sigma(e-)^2 = sigma^2 - 36.98^2 = a^2*Erec+b^2*Erec^2 - 36.98^2 = a^2*(Ee+Erec(0)) + b^2*(Ee+Erec(0))^2 - 36.98^2
...
double get_ratio(double *X,double *p){
		...
		for(int i=0;i<N;i++){
			...
			for(int j=0;j<nEle;j++){
				...
				totalPE[i] += Erec;
				Erec += 1255.02/1.3056438e3;
				sum_sigma_2 += p[0]*p[0]*Erec + p[1]*p[1]*Erec*Erec - 36.98*36.98/1.3056438e3/1.3056438e3;
			}
		}
...
-0.0617164};
	...
	TF1 *f1 = new TF1("f1",get_ratio,0,8,7);
	f1->SetParameters(0.02803,0.007886,1.07476,0.00148426,-0.001801,0.090052,1.30358);

	f1->SetParNames("a","b","P_{0}","P_{1}","P_{2}","P_{3}","P_{4}");

    f1->FixParameter(2,1.07476);
    f1->FixParameter(3,0.00148426);
    f1->FixParameter(4,-0.001801);
    f1->FixParameter(5,0.090052);
    f1->FixParameter(6,1.30358);
