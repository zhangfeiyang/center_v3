...
const int N = 10000;
double totalPE[N];
double data_x[9],data_y[9],edata[9];
...
void myfcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag){
	...
	for(int i=0;i<9;i++){
		X = data_x[i];
		predict[i] = get_ratio(&X,par);
		chi2 += ((data_y[i]-predict[i])/edata[i])*((data_y[i]-predict[i])/edata[i]);
	}

	chi2 += (par[3]-1.07476)*(par[3]-1.07476)/0.01/0.01+(par[4]-0.00148426)*(par[4]-0.00148426)/1.5e-4/1.5e-4;	
	f = chi2;
...
void fit5(){
	...
	double par[8] = {0.027,0.02803,0.007886,1.07476,0.00148426,-0.001801,0.090052,1.30358};
	double step[8];

	for(int i=0;i<8;i++){
    	gMinuit->mnparm(i,"Constant",par[i],1e-6,0,0,ierflg);
	} 
    arglist[0] = 10000;
    arglist[1] = 1.;
	...
	for(int i=0;i<8;i++){
		gMinuit->GetParameter(i,result,eresult);
		par[i] = result;
		fout<<result<<"\t";
	}
